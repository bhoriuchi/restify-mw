...
// Modified by Branden Horiuchi <bhoriuchi@gmail.com>
...
function serveStatic(opts) {
    ...
    assert.optionalString(opts.virtual, 'options.virtual');
    ...
    function serve(req, res, next) {
        ...
        /**
         * Modified by Branden Horiuchi <bhoriuchi@gmail.com>
         * Adding a virtual option to allow for virtual directories
         * this allows you to remove the virtual part in the request
         * path so that http://server/virtualpath/dir/file can be located 
         * in /directory/dir/file instead of the default 
         * /directory/virtualpath/dir/file
         */
        else if (opts.virtual) {
        	
        	// create a regex with the virtual path
        	var virtRx = new RegExp('^' + opts.virtual);
        	
        	// set the file as <the directory> + (<route path> - <virtual path>)
        	file = path.join(opts.directory,
        			decodeURIComponent(req.path().replace(virtRx, '')));
        }
        else {
            file = path.join(opts.directory,
